...
title = st.title(r"$y_d(T, P, L, G, y^+)$ for Water")

T = st.text_input("Temperature (K):", key='T', value = '0.0')

P = st.text_input("Pressure ($Pa$):", key = 'P', value = '0.0')

L = st.text_input("Characteristic length of the flow ($m$):", key = 'L', value = '0.0')

G = st.text_input("Mass velocity of the flow ($kg/m^2 s$):", key = 'U', value = '0.0')
